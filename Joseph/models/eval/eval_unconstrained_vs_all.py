"""Compare DDPM, V-CNN, StreamDDPM(full), DivergentUNet, and the new
UnconstrainedStream model on the same 100 seeds / same real chronological
ground truth / same biased-walk sparse sampling as all earlier sweeps.
"""
import csv
import pickle
import sys
...
for seed in range(N_SEEDS):
    ...
    print(f"seed {seed:3d} frame={fi:5d} n_obs={n_obs:4d}  "
          ...
          f"UNCONSTRAINED[rmse={u_rmse:.4f} ang={u_ang:6.2f}]", flush=True)
    ...
    if seed in IMAGE_SEEDS:
        ...
        plt.tight_layout()
        out = RESULTS / f"unconstrained_vs_all_seed{seed}.png"
        plt.savefig(out, dpi=130)
        plt.close(fig)
        print(f"  saved {out}", flush=True)

csv_path = RESULTS / "unconstrained_vs_all_metrics.csv"
...
for model_name in ("ddpm", "vcnn", "stream_full", "divergentunet", "unconstrained"):
