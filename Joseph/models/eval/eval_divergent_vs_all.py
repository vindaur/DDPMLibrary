"""Compare DDPM, V-CNN, StreamDDPM (+full_field), and the new raw-trained
DivergentUNet on the same 100 seeds / same real chronological ground truth /
same biased-walk sparse sampling as the earlier apples-to-apples sweep.
"""
import csv
import pickle
import sys
...
land_mask = np.asarray(d["land_mask"]).astype(bool)      # (94, 94) True=land
...
for seed in range(N_SEEDS):
    ...
    oi, oj = np.where(path_mask & ~land_mask)
    n_obs = len(oi)
    obs = [
    ...
    print(f"seed {seed:3d} frame={fi:5d} n_obs={n_obs:4d}  "
    ...
    if seed in IMAGE_SEEDS:
        ...
        plt.tight_layout()
        out = RESULTS / f"divergent_vs_all_seed{seed}.png"
        plt.savefig(out, dpi=130)
        plt.close(fig)
        print(f"  saved {out}")

csv_path = RESULTS / "divergent_vs_all_metrics.csv"
...
for model_name in ("ddpm", "vcnn", "stream_full", "divergentunet"):
