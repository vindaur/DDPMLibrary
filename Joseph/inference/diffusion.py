...
class DDPM:
    ...
    def training_loss(
        ...
        cond: optional (B, C_cond, H, W) conditioning tensor (e.g. sparse
              observation + path mask, Palette-style) concatenated onto xt
              before the model call. None reproduces the original
              unconditional behavior exactly.
        """
        B = x0.shape[0]
        t = torch.randint(0, self.T, (B,), device=self.device)
        xt, noise = self.q_sample(x0, t)
        model_in = xt if cond is None else torch.cat([xt, cond], dim=1)
