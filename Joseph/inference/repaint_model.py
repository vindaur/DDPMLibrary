...
class Repaint(nn.Module):
    ...
    def __init__(self, in_ch: int = 2, base_ch: int = 64, time_dim: int = 256,
        ...
        """
        in_ch:  network input channels (2 for plain xt; > 2 when extra
                conditioning channels — e.g. observation + mask, Palette-style
                — are concatenated onto xt before the forward call).
        out_ch: denoising target channels. Defaults to in_ch (old behavior,
                every existing unconditional checkpoint/script is unaffected).
        """
        super().__init__()
