...
class Repaint(nn.Module):
    ...
    cond_ch (optional): extra per-pixel conditioning channels (e.g. temporal
    priors -- see chrono_dataset.py), concatenated onto the noisy field
    before the first conv. cond_ch=0 (default) reproduces the original
    unconditional model exactly.
    """
    ...
    def __init__(self, in_ch: int = 2, cond_ch: int = 0, base_ch: int = 64, time_dim: int = 256):
        super().__init__()
        ...
        # Encoder
        self.enc0 = ResBlock(in_ch + cond_ch, c, time_dim)   # 96×48
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor,
        ...
        Args:
            ...
            cond: (B, cond_ch, 94, 44) optional conditioning channels
                  (required if this model was built with cond_ch > 0)
        Returns:
        ...
        if self.cond_ch > 0:
            if cond is None:
                raise ValueError(
                    f"model built with cond_ch={self.cond_ch} but forward() got cond=None")
